...
- (void)didFinishPickingPhoto:(NSArray<XMNAssetModel *> *)assets {
    ...
    if (_height == 1) {
        ...
        _cutVC.ImgHeight = JKMainScreenSize().width;
    }else {
        _cutVC.ImgWidth = JKMainScreenSize().width;
        _cutVC.ImgHeight = _height;
    }
    [_cutVC
     cutImage:images completionHandler:^(NSArray *finishArr) {
         ...
         if (_finishImgArr.count>0) {
             if (self.photoPickerDelegate && [self.photoPickerDelegate respondsToSelector:@selector(photoPickerController:didFinishPickingPhotos:sourceAssets:)]) {
                 [self.photoPickerDelegate photoPickerController:self didFinishPickingPhotos:_finishImgArr sourceAssets:assets];
             }
             self.didFinishPickingPhotosBlock ? self.didFinishPickingPhotosBlock(_finishImgArr,assets) : nil;
         }
         
         
         
     }];
    [self pushViewController:_cutVC animated:YES ];
