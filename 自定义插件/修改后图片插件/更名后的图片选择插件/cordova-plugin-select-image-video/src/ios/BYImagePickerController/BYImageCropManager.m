...
@implementation BYImageCropManager
...
+ (UIImage *)cropImageView:(UIImageView *)imageView toRect:(CGRect)rect zoomScale:(double)zoomScale containerView:(UIView *)containerView {
    ...
    CGFloat xMargin = containerView.BY_width - CGRectGetMaxX(rect) - rect.origin.x;
    ...
    cropedImage = [[BYImageManager manager] fixOrientation:cropedImage];
...
@implementation UIImage (BYGif)
    
+ (UIImage *)sd_BY_animatedGIFWithData:(NSData *)data {
    if (!data) {
        return nil;
    }
