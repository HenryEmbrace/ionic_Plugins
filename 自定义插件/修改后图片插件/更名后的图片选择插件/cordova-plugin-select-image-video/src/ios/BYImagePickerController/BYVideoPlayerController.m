...
@interface BYVideoPlayerController () {
    AVPlayer *_player;
...
@implementation BYVideoPlayerController
...
- (void)viewDidLoad {
    ...
    if (BYImagePickerVc) {
        self.navigationItem.title = BYImagePickerVc.previewBtnTitleStr;
    }
...
- (void)configMoviePlayer {
    [[BYImageManager manager] getPhotoWithAsset:_model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
        _cover = photo;
    }];
    [[BYImageManager manager] getVideoWithAsset:_model.asset completion:^(AVPlayerItem *playerItem, NSDictionary *info) {
...
- (void)configBottomToolBar {
    ...
    if (BYImagePickerVc) {
        ...
        [_doneButton setTitle:[NSBundle BY_localizedStringForKey:@"Done"] forState:UIControlStateNormal];
...
- (void)viewDidLayoutSubviews {
    ...
    _playerLayer.frame = self.view.bounds;
    _playButton.frame = CGRectMake(0, 64, self.view.BY_width, self.view.BY_height - 64 - 44);
