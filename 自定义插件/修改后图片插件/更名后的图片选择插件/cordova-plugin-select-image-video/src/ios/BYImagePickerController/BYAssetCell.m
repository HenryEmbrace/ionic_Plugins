...
@implementation BYAssetCell

    - (void)setModel:(BYAssetModel *)model {
        _model = model;
        if (iOS8Later) {
            self.representedAssetIdentifier = [[BYImageManager manager] getAssetIdentifier:model.asset];
        }
        int32_t imageRequestID = [[BYImageManager manager] getPhotoWithAsset:model.asset photoWidth:self.BY_width completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
            if (_progressView) {
            ...
            if ([self.representedAssetIdentifier isEqualToString:[[BYImageManager manager] getAssetIdentifier:model.asset]]) {
...
- (void)setType:(BYAssetCellType)type {
    _type = type;
    if (type == BYAssetCellTypePhoto || type == BYAssetCellTypeLivePhoto || (type == BYAssetCellTypePhotoGif && !self.allowPickingGif) || self.allowPickingMultipleVideo) {
    ...
    if (type == BYAssetCellTypeVideo) {
        self.bottomView.hidden = NO;
        self.timeLength.text = _model.timeLength;
        self.videoImgView.hidden = NO;
        _timeLength.BY_left = self.videoImgView.BY_right;
        _timeLength.textAlignment = NSTextAlignmentRight;
    } else if (type == BYAssetCellTypePhotoGif && self.allowPickingGif) {
        self.bottomView.hidden = NO;
        self.timeLength.text = @"GIF";
        self.videoImgView.hidden = YES;
        _timeLength.BY_left = 5;
...
- (void)selectPhotoButtonClick:(UIButton *)sender {
    ...
    if (sender.isSelected) {
        [UIView showOscillatoryAnimationWithLayer:_selectImageView.layer type:BYOscillatoryAnimationToBigger];
        // 用户选中了该图片，提前获取一下大图
        [self fetchBigImage];
    } else { // 取消选中，取消大图的获取
...
- (void)fetchBigImage {
    _bigImageRequestID = [[BYImageManager manager] getPhotoWithAsset:_model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
        if (_progressView) {
            [self hideProgressView];
        }
...
- (void)layoutSubviews {
    ...
    if (self.allowPreview) {
        _selectPhotoButton.frame = CGRectMake(self.BY_width - 44, 0, 44, 44);
    } else {
        _selectPhotoButton.frame = self.bounds;
    }
    _selectImageView.frame = CGRectMake(self.BY_width - 27, 0, 27, 27);
    _imageView.frame = CGRectMake(0, 0, self.BY_width, self.BY_height);
    
    static CGFloat progressWH = 20;
    CGFloat progressXY = (self.BY_width - progressWH) / 2;
    _progressView.frame = CGRectMake(progressXY, progressXY, progressWH, progressWH);
    
    _bottomView.frame = CGRectMake(0, self.BY_height - 17, self.BY_width, 17);
    _videoImgView.frame = CGRectMake(8, 0, 17, 17);
    _timeLength.frame = CGRectMake(self.videoImgView.BY_right, 0, self.BY_width - self.videoImgView.BY_right - 5, 17);
...
- (void)setModel:(BYAlbumModel *)model {
    _model = model;
    ...
    [[BYImageManager manager] getPostImageWithAlbumModel:model completion:^(UIImage *postImage) {
...
- (void)layoutSubviews {
    ...
    _selectedCountButton.frame = CGRectMake(self.BY_width - 24 - 30, 23, 24, 24);
    NSInteger titleHeight = ceil(self.titleLabel.font.lineHeight);
    self.titleLabel.frame = CGRectMake(80, (self.BY_height - titleHeight) / 2, self.BY_width - 80 - 50, titleHeight);
...
@implementation BYAssetCameraCell
