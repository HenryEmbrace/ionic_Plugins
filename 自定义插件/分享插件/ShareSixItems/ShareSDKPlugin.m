...
-(void)shareWithContent:(id)publishContent/*只需要在分享按钮事件中 构建好分享内容publishContent传过来就好了*/
{
    ...
    UIView *shareView = [[UIView alloc] initWithFrame:CGRectMake(0,(kScreenHight - 2 *bt_width - 10)  ,kScreenWidth,2 *bt_width)];
    
    shareView.tag = 441;
    [window addSubview:shareView];
    NSArray *btnImages = @[@"sns_icon_24@2x", @"sns_icon_22@2x", @"sns_icon_6@2x", @"sns_icon_23@2x",@"sns_icon_37@2x",@"sns_icon_1@2x"];
    NSArray *btnTitles = @[ @"QQ", @"微信好友", @"QQ空间", @"朋友圈",@"微信收藏",@"微博"];
    
    for (NSInteger i=0; i<btnImages.count; i++) {
        NSInteger index = i % 3;
        NSInteger page = i / 3;
        CGFloat gap = (shareView.frame.size.width - 3*bt_width) / 3.0;
        CGFloat top = 0.0f;
        if (i<3) {
            top = 0*KWidth_Scale;
        ...
        UIButton *button = [[UIButton alloc] initWithFrame:CGRectMake(index * (bt_width + gap) + Start_X, page  * (bt_width), bt_width, bt_width)];
