...
- (void)passRecordingMethod:(CDVInvokedUrlCommand *)command {
    ...
    UIView *shareView = [[UIView alloc] initWithFrame:CGRectMake(20,kScreenHight - 60,kScreenWidth - 40,60)];
    ...
    self.recordButton.frame = CGRectMake(0, 0, kScreenWidth - 40, 40);
...
-(void)toDoRecord:(id)command
{
    ...
    self.recordButton.recordTouchUpInsideAction = ^(MOKORecordButton *sender){
        ...
        [self.commandDelegate sendPluginResult:[CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:self.str] callbackId:command];
